(function () {
    ...
    function ready(cb) {
        if (window.Lampa && window.Lampa.Activity && window.Lampa.Component && window.Lampa.Select && window.Lampa.Bookmarks) {
            cb();
        } else {
            setTimeout(() => ready(cb), 300);
        }
    }
    ...
    ready(function () {
        ...
        function saveFolders(folders) { localStorage.setItem(STORAGE_KEY, JSON.stringify(folders)); }
        ...
        Lampa.Select.show = function (params) {
            try {
                ...
                if (isFav || (params.title && (params.title.indexOf('Вибране') !== -1 || params.title.indexOf('Избранное') !== -1))) {
                    ...
                    var movie = act ? act.card || act.data : null;

                    if (folders.length && movie) {
                        params.items.push({ title: '--- МОЇ ПАПКИ ---', separator: true });
                        folders.forEach(function (f, i) { params.items.push({ title: '📁 ' + f.name, is_custom: true, f_idx: i }); });
            ...
            } catch(e){ console.error('CustomBookmarks error', e); }
