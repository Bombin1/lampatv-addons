(function () {
    ...
    if (!$('#custom-bookmarks-styles').length) {
        $('body').append('<style id="custom-bookmarks-styles"> \
            ...
            .folder-tile { background: rgba(255, 255, 255, 0.08); width: 85px; height: 50px; border-radius: 6px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; border: 1px solid transparent; } \
            .folder-tile.focus { background: #fff !important; color: #000 !important; transform: scale(1.05); } \
            .folder-tile__name { font-size: 0.7em; font-weight: 500; text-align: center; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; width: 100%; } \
            .folder-tile__count { font-size: 0.65em; opacity: 0.5; } \
            .folder-tile--create { border: 1px dashed rgba(255, 255, 255, 0.2); } \
        </style>');
    }

    // 1. ВІДОБРАЖЕННЯ В ЗАКЛАДКАХ
                    ...
                    if (container.length) {
                        ...
                        folders.forEach(function(folder, i) {
                            ...
                            tile.on('click', function() {
                                // ПРАВИЛЬНИЙ ФОРМАТ ДЛЯ category_full
                                Lampa.Activity.push({
                                    title: folder.name,
                                    component: 'category_full',
                                    cards: folder.list, // Передаємо масив сюди
                                    page: 1,
                                    method: 'card' // Вказуємо, що це тип "картки"
                                });
                            });
            ...
            if (folders.length > 0 && movie) {
                ...
                params.onSelect = function (item) {
                    if (item.is_custom) {
                        ...
                        // Копіюємо об'єкт без зайвих зв'язків
